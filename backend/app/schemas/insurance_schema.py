...
from app.models.insurance import Insurance, PatientInsurance
...
class InsuranceSchema(Schema):
    ...
    ville = fields.Str(allow_none=True)
    code_postal = fields.Str(allow_none=True)
    telephone = fields.Str(allow_none=True)
    email = fields.Email(allow_none=True)
    site_web = fields.Str(allow_none=True)
    
    contact_nom = fields.Str(allow_none=True)
    contact_telephone = fields.Str(allow_none=True)
    contact_email = fields.Email(allow_none=True)
    
    delai_paiement = fields.Int(allow_none=True)
...
class PatientInsuranceSchema(Schema):
    ...
    assurance_id = fields.Int(required=True)
    
    numero_adherent = fields.Str(required=True)
    date_debut = fields.Date(required=True)
    date_fin = fields.Date(allow_none=True)
    taux_couverture = fields.Float(allow_none=True)
    plafond_annuel = fields.Float(allow_none=True)
    ...
    assurance = fields.Nested('InsuranceSchema', only=('id', 'nom', 'type'), dump_only=True)

patient_insurance_schema = PatientInsuranceSchema()
patient_insurances_schema = PatientInsuranceSchema(many=True)
